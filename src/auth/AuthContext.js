...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    console.log('Restoring session - Stored User:', storedUser, 'Timestamp:', sessionTimestamp, 'SessionId:', storedSessionId); // Debug
    ...
    if (storedUser && sessionTimestamp && storedSessionId) {
      ...
      if (timeElapsed < SESSION_TIMEOUT) {
        ...
        setSessionActive(true);
        sessionIdRef.current = storedSessionId;
        console.log('Session restored for user:', parsedUser); // Debug
      } else {
        clearSessionData();
        console.log('Session expired, cleared data'); // Debug
      }
    }
    setInitializing(false); // Mark initialization as complete
  }, []);
  ...
  const logout = useCallback(async () => {
    ...
    sessionIdRef.current = null;
    clearSessionData();
    console.log('Logged out and session cleared'); // Debug
  }, [user]);

  const resetSessionTimeout = useCallback(() => {
    localStorage.setItem('sessionTimestamp', Date.now().toString());
    console.log('Session timeout reset at:', new Date().toISOString()); // Debug
  }, []);
  ...
  const login = async (userData) => {
    console.log('Attempting login with userData:', userData); // Debug
    const sessionId = generateSessionId();
    const minimalUserData = {
      name: userData.name || 'Unknown', // Default to 'Unknown' if name is missing
      employeeID: userData.employeeID,
      phone: userData.phone,
      role: userData.role,
      sessionId: sessionId,
    ...
    try {
      ...
      console.log('Login successful, user set:', minimalUserData); // Debug
    } catch (error) {
      console.error("Error logging login time:", error);
      // Optionally handle error (e.g., logout or alert), but proceed with login
    }
  };
