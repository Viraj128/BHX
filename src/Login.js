...
const Login = () => {
  ...
  const handlePhoneSubmit = async (e) => {
    ...
    try {
      ...
      // Check if OTP exists in userData (assuming it's stored in Firestore)
      if (!userData.otp) {
        throw new Error("OTP not defined for this user");
      }

      setUserData({ ...userData, id: userDoc.id, employeeID: userData.employeeID });
      setStep("otp");
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      console.log("Verifying OTP for userData:", userData); // Debug log
      if (otp === userData.otp.toString()) {
        // Successful login
        const loggedInUser = {
          ...userData,
        ...
        console.log("Logging in with user:", loggedInUser); // Debug log
        await login(loggedInUser);
        localStorage.setItem('sessionTimestamp', Date.now().toString()); // Set session timestamp
        navigate(getDashboardPath(), { replace: true }); // Navigate after login
      } else {
