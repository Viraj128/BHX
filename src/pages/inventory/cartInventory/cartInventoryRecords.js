...
const CartInventoryRecords = () => {
  ...
  useEffect(() => {
    const fetchInventory = async () => {
      try {
        setLoading(true);
        const querySnapshot = await getDocs(collection(db, 'inventory_02'));
  ...
  const handleSave = async (itemId) => {
    ...
    try {
      ...
      const itemDocRef = doc(db, 'inventory_02', itemId);
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      await deleteDoc(doc(db, 'inventory_02', itemId));
...
export default CartInventoryRecords;