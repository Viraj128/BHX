...
const KartCategories = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const catRef = collection(db, "category_02"); // Changed from "category" to "category_02"
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "category_02", newId), { // Changed from "category" to "category_02"
        id: newId,
        name: newCategory,
        active: true,
      });
  ...
  const handleDeleteCategory = async (firestoreId) => {
    ...
    try {
      await deleteDoc(doc(db, "category_02", firestoreId)); // Changed from "category" to "category_02"
  ...
  const handleUpdateCategory = async (firestoreId) => {
    ...
    try {
      ...
      if (editingIdValue !== originalCategory.id) {
        ...
        await setDoc(doc(db, "category_02", editingIdValue), { // Changed from "category" to "category_02"
          id: editingIdValue,
          name: editingName,
        ...
        const docRef = doc(db, "category_02", firestoreId); // Changed from "category" to "category_02"
