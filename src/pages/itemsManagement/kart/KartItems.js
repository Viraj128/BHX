...
const KartItems = () => {
  ...
  useEffect(() => {
    if (isAdmin || isManager || isTeamLeader) {
      const unsubscribe = onSnapshot(collection(db, 'items_02'), snapshot => { // Changed from 'items' to 'items_02'
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      const snapshot = await getDocs(collection(db, 'category_02')); // Changed from 'category' to 'category_02'
  ...
  useEffect(() => {
    const fetchSauceGroups = async () => {
      const snapshot = await getDocs(collection(db, 'sauceGroups_02')); // Changed from 'sauceGroups' to 'sauceGroups_02'
  ...
  const handleSubmit = async e => {
    ...
    const itemData = {
      ...
      categoryId: form.categoryId ? doc(db, 'category_02', form.categoryId) : null, // Changed from 'category' to 'category_02'
      sauces: form.sauceGroupId ? doc(db, 'sauceGroups_02', form.sauceGroupId) : null, // Changed from 'sauceGroups' to 'sauceGroups_02'
    ...
    try {
      if (editId) {
        await updateDoc(doc(db, 'items_02', editId), itemData); // Changed from 'items' to 'items_02'
      } else {
        const snapshot = await getDocs(collection(db, 'items_02')); // Changed from 'items' to 'items_02'
        const newId = `item${(snapshot.size + 1).toString().padStart(2, '0')}`;
        await setDoc(doc(db, 'items_02', newId), { ...itemData, id: newId }); // Changed from 'items' to 'items_02'
      }
  ...
  const handleDelete = async id => {
    ...
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        await deleteDoc(doc(db, 'items_02', id)); // Changed from 'items' to 'items_02'
      } catch (error) {
  ...
  const handleToggleActive = async (id, currentActive) => {
    try {
      await updateDoc(doc(db, 'items_02', id), { active: !currentActive }); // Changed from 'items' to 'items_02'
    } catch (error) {
