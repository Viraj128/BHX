...
const KartSauces = () => {
  ...
  useEffect(() => {
    if (isAdmin || isManager || isTeamLeader) {
      setLoading(true);
      const unsubscribe = onSnapshot(collection(db, "sauceGroups_02"), (snapshot) => { // Changed from "sauceGroups" to "sauceGroups_02"
  ...
  const toggleActive = async (groupId, currentActive) => {
    try {
      ...
      const docRef = doc(db, 'sauceGroups_02', groupId); // Changed from "sauceGroups" to "sauceGroups_02"
  ...
  const handleAddGroup = async () => {
    ...
    try {
      ...
      const docRef = doc(db, 'sauceGroups_02', categoryName); // Changed from "sauceGroups" to "sauceGroups_02"
  ...
  const handleUpdateGroup = async (id) => {
    ...
    try {
      ...
      const docRef = doc(db, 'sauceGroups_02', id); // Changed from "sauceGroups" to "sauceGroups_02"
  ...
  const handleDeleteGroup = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, 'sauceGroups_02', id)); // Changed from "sauceGroups" to "sauceGroups_02"
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
        <div className="p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Cart(Liverpool One) Sauce Group Management</h1>
              <p className="text-gray-600 mt-1">
                Manage your kart sauce groups and organization
              </p>
            </div>
