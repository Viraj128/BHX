...
// Without Phone number search Directly Details is display 
import React, { useState, useEffect } from "react";
import { collection, query, where, getDocs } from "firebase/firestore";
import { db } from '../../firebase/config';
import { useAuth } from '../../auth/AuthContext';
import { FiUser, FiPhone, FiMail, FiHome, FiFileText, FiClock, FiBriefcase } from "react-icons/fi";
...
const ViewDetails = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        console.log(`Fetching details for phone: ${user.phone}`);

        // Search in users_01 collection
        console.log(`Querying users_01 for phone: ${user.phone}`);
        const usersQuery = query(collection(db, "users_01"), where("phone", "==", user.phone));
        const userSnapshot = await getDocs(usersQuery);

        if (userSnapshot.empty) {
          console.log(`No user found in users_01 for phone: ${user.phone}`);
          setNotFound(true);
          setLoading(false);
          return;
        }
        ...
        // Search in customers collection
        console.log(`Querying customers for phone: ${user.phone}`);
        const customersQuery = query(collection(db, "customers"), where("phone", "==", user.phone));
        const customerSnapshot = await getDocs(customersQuery);
        ...
        if (!customerSnapshot.empty) {
          ...
          console.warn(`No customer found in customers for phone: ${user.phone}`);
        }
