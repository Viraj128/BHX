...
//       await fetchAdminAttendanceData(date); // Re-fetch data
...
const Attendance = () => {
  ...
  // Permission flags
  const showActionsColumn = (isAdmin && isEditableDate) || ((isManager || isTeamLeader) && isToday);
  const canAddNew = isAdmin && isEditableDate;
  const canEdit = (isAdmin && isEditableDate) || ((isManager || isTeamLeader) && isToday);
  const canDelete = isAdmin && isEditableDate;
